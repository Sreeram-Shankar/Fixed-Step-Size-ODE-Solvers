...
function legendre_roots_and_weights(n::Int)
    ...
    #transforms from the Gaussian quadrature interval to the rk interval
    c = [(x + 1) / 2 for x in xs]
    b = [w / 2 for w in ws]
    
    #sorts the nodes by their c values
    perm = sortperm(c)
...
#builds the gauss-legendre irk tableau
function build_gauss_legendre_irk(s::Int)
